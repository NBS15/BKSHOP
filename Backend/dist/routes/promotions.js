...
router.get('/active', (req, res) => {
    const now = new Date();
    const activePromos = promotions.filter((p) => p.active && p.startDate <= now && p.endDate >= now);
    res.json(activePromos);
});
// GET promotion by code
router.get('/code/:code', (req, res) => {
    const promotion = promotions.find((p) => p.code.toUpperCase() === req.params.code.toUpperCase());
    if (!promotion) {
